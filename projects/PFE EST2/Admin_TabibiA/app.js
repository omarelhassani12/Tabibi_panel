...
// Define the route for updating user activation
app.post('/update-user-activation', (req, res) => {
  const { userId, isActive } = req.body;

  // Update the user's activation status in the database
  const query = `UPDATE users SET isActive = ${isActive ? 1 : 0} WHERE id = ${userId}`;
  ...
  db.query(query, (error, results) => {
    ...
    return res.json({ message: 'User activation status updated successfully' });
  });
});
